...
echo "Ubuntu 分版本的安装脚本：https://github.com/MABIY/shell/blob/master/ubuntu_22.04_init_install_update.sh"
...
4：   安装terminator(终端模拟器)
5：   安装搜狗输入法(默认安装)
6：   安装VS Code(代码编辑器，默认安装)
7：   安装Sublime Text(代码编辑器，默认安装)
8：   安装ReText(Markdown编辑器，默认安装)
9：   安装Chrome(谷歌浏览器，默认安装)
10：  安装Meld(文件比较工具，默认安装)
11：  安装Kazam(屏幕录像工具，默认安装)
